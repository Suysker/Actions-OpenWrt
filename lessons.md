...
- GCC 与 package 兼容规则：用户明确允许并要求把 `CONFIG_GCC_USE_VERSION_15=y` 作为稳定工具链契约写死；不得擅自改成 Lean 默认 GCC13、自动最高 major 或失败回退。GCC 15 暴露旧 package 问题时先查 canonical 上游和本轮锁定的官方 `openwrt/packages`：Lean feed 落后而官方 recipe 已吸收修复时，把对应 `<category>/<package>` 加入 common 的唯一子树 allowlist，与 Go 共用一次解析、source-lock 和稀疏同步；当前 `nlbwmon` 的 buffer 修复与 `libwebsockets` 的 ChaCha 数组长度修复都走这条路径，不复制版本/hash 或短期 backport。只有官方也未修复时才评审窄语义兼容；`libsepol` 的 GNU17 兼容只能通过唯一 package include 锚点幂等插入。不得全局降级 GCC、关闭 `-Werror` 或放宽错误。
