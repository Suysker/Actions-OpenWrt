...
- GCC 与 package 兼容规则：用户明确允许并要求把 `CONFIG_GCC_USE_VERSION_15=y` 作为稳定工具链契约写死；不得擅自改成 Lean 默认 GCC13、自动最高 major 或失败回退。GCC 15 暴露旧 package 问题时先查 canonical 上游和对应 OpenWrt 官方 master：Lean core/feed 落后而官方 recipe 已吸收修复时，在 common 的唯一 `source-overlays.json` 中声明窄 source→target 映射；resolver 按仓库一次解析并锁定 commit，通用同步器按仓库一次稀疏 checkout，执行代码不得枚举包名。当前 `libtirpc` C23 原型、`nlbwmon` buffer、`libwebsockets` ChaCha 数组和 GMP C23 compiler probe 修复都走这条路径，不复制版本/hash 或短期 backport。只有官方也未修复时才评审窄语义兼容；`libsepol` 的 GNU17 兼容只能通过唯一 package include 锚点幂等插入。不得全局降级 GCC、关闭 `-Werror` 或放宽错误。
