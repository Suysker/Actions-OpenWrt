...
- GCC 与 package 兼容规则：用户明确允许并要求把 `CONFIG_GCC_USE_VERSION_15=y` 作为稳定工具链契约写死；不得擅自改成 Lean 默认 GCC13、自动最高 major 或失败回退。GCC 15 暴露旧 package 问题时先看 canonical 上游是否已有修复：`nlbwmon` 应复用本轮锁定的官方 `openwrt/packages/net/nlbwmon`，Go 与它共用一次仓库同步；`libsepol` 的 GNU17 兼容只能通过唯一 package include 锚点幂等插入，不把 `PKG_VERSION`、`PKG_HASH` 或 release URL 复制进仓库 patch 上下文。不得全局降级 GCC、关闭 `-Werror` 或放宽错误。
